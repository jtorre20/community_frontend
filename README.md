...
Open [http://https://secure-taiga-49628.herokuapp.com](http://https://secure-taiga-49628.herokuapp.com) to view it in the browser.
...
```json
{
  ...
  "configurations": [{
    ...
    "type": "chrome",
    "request": "launch",
    "url": "http://https://secure-taiga-49628.herokuapp.com",
...
In the WebStorm menu `Run` select `Edit Configurations...`. Then click `+` and select `JavaScript Debug`. Paste `http://https://secure-taiga-49628.herokuapp.com` into the URL field and save the configuration.
...
```
Fetch API cannot load http://localhost:4000/api/todos. No 'Access-Control-Allow-Origin' header is present on the requested resource. Origin 'http://https://secure-taiga-49628.herokuapp.com' is therefore not allowed access. If an opaque response serves your needs, set the request's mode to 'no-cors' to fetch the resource with CORS disabled.
```
...
If you use routers that use the HTML5 [`pushState` history API](https://developer.mozilla.org/en-US/docs/Web/API/History_API#Adding_and_modifying_history_entries) under the hood (for example, [React Router](https://github.com/ReactTraining/react-router) with `browserHistory`), many static file servers will fail. For example, if you used React Router with a route for `/todos/42`, the development server will respond to `https://secure-taiga-49628.herokuapp.com/todos/42` properly, but an Express serving a production build as above will not.
